...
if (version$os == "mingw32")
{
  ...
  if (is.na(match("ArrayTestPas",  attr(getLoadedDLLs(), "names"))))
  {
    dyn.load("./ArrayTestPas.dll")
  }

} else
{
  # Running in Linux
  if (is.na(match("libArrayTestPas",  attr(getLoadedDLLs(), "names"))))
  {
    dyn.load("./libArrayTestPas.so")
  }
}
...
Context <- AT.create(AT.ix, AT.iy, AT.A1_I, AT.A1_D, AT.A2_IA, AT.A2_IB)
