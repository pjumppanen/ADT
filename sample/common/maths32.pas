...
CONST Euler_gamma: DOUBLE = 0.577215664901532860606512;
      ...
      d_signan_data: QWORD = $FFF7FFFFFFF7FFFF; // repeat
      s_signan_data: LONGWORD = $FFBFFFFF;
      inv_256 = 1/256;

VAR infinity: SINGLE ABSOLUTE infinity_data;
    qnan: SINGLE ABSOLUTE qnan_data;
    snan_d: DOUBLE ABSOLUTE d_signan_data; // not sure you can ever USE these!
    snan_s: SINGLE ABSOLUTE s_signan_data;
    pausing: BOOLEAN;
...
PROCEDURE MULBY(
  VAR x: DOUBLE;
  CONST by: DOUBLE); OVERLOAD; INLINE;

PROCEDURE MULBY(
  VAR x: INTEGER;
  CONST by: INTEGER); OVERLOAD; INLINE;

PROCEDURE DIVBY(
  VAR x: DOUBLE;
  CONST by: DOUBLE); OVERLOAD; INLINE;

// No DIVBY for integers!
...
{$IFDEF FPC} // redeclare from MATH.PP
FUNCTION MAX( CONST x1, x2: DOUBLE):
  DOUBLE; OVERLOAD; INLINE;

FUNCTION MIN( CONST x1, x2: DOUBLE):
  DOUBLE; OVERLOAD; INLINE;
{$ENDIF}
...
FUNCTION sign( apply_to, apply_from: DOUBLE): DOUBLE; INLINE; OVERLOAD;
...
FUNCTION nonzero( CONST x: INTEGER): DOUBLE; INLINE; OVERLOAD;

FUNCTION nonzero( CONST x: DOUBLE): DOUBLE; INLINE; OVERLOAD;

FUNCTION n_log_p( CONST n, p: DOUBLE): DOUBLE; INLINE;

FUNCTION dr_n_log_p( CONST r: INTEGER; CONST n, p: DOUBLE): DOUBLE;
...
CONST
  max_factor=10;
  factorial: ARRAY[ 0..max_factor] OF INTEGER =
...
{$ENDIF} {OBSOLETE_MATHS32}

PROCEDURE set_signan(
  VAR x: DOUBLE;
  CONST n: INTEGER);
BEGIN
  FillQword( x, n, d_signan_data);
END;
...
PROCEDURE set_signan(
  ...
  CONST n: INTEGER);
BEGIN
  FillDword( x, n, $FFFFFFFF); // this doesn't work, of course, but is a "funny number"
END;
...
PROCEDURE set_signan(
  ...
  CONST n: INTEGER);
BEGIN
  FillDword( x, n, $BAD00BAD); // this doesn't work, of course, but is a "funny number"
END;
...
FUNCTION EXP(
  ...
  DOUBLE; INLINE;
BEGIN
  IF isInfinite(x) AND (Math.SIGN(x)=-1) THEN
    RESULT:= 0
  ELSE
    RESULT:= System. EXP( x);
END;
...
CONST
  nonzero_data: ARRAY[ FALSE..TRUE] OF DOUBLE = (0,1);

FUNCTION nonzero(
    CONST x: INTEGER):
  DOUBLE;
BEGIN
  RESULT:= nonzero_data[ x<>0];
END;
...
FUNCTION n_log_p(
  ...
  DOUBLE;
BEGIN
  RESULT:= n * LN( p + BYTE(n=0)); // guaranteed 0 if n=0
END;
...
VAR
  rmax, i_r: INTEGER;
BEGIN
  rmax:= LMIN( r-1, max_factor);
  RESULT:= -factorial[ rmax] * n / ipwr( -(p + BYTE(n=0)), r);
  FOR i_r:= max_factor+1 TO r-1 DO BEGIN
    RESULT *= i_r;
  END;
END;
...
BEGIN
  max_del:= 0;
  FOR t:= 1 TO nt DO
    max_del:= Math. max( max_del, rel_delta( el( old_vals, t), el( new_vals, t)));
...
FUNCTION cum_normal_prob( x: DOUBLE):
         DOUBLE;
BEGIN
  cum_normal_prob:= 1 - 0.5 * erfcc( x * SQRT_half);
END;
...
FUNCTION dnorm(
  ...
  DOUBLE;
BEGIN
   dnorm:= inv_sqrt_two_pi * EXP( -0.5 * SQR( x));
END;
