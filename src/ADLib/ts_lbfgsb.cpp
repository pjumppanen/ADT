/*
 Hand modified version of lbfgsb.c source code in R project that makes the
...
struct ts_setulb_state
{
  /* Local variables */
  int lsnd, ld, lr, lt, lz, lwa, lwn, lss, lws, lwt, lsy, lwy;
} ts_setulb_state;


struct ts_mainlb_state
{
  // formerly lsave
  int prjctd, cnstnd, boxed, updatd;
  // in isave
  int nintol, iback, nskip, head, col, itail, iter, iupdat,
      nint, nfgv, info, ifun, iword, nfree, nact, ileave, nenter;
  // formerly dsave
  double theta, fold, tol, dnorm, epsmch, gd, stpmx, sbgnrm,
      stp, gdold, dtd;
};


struct ts_dcsrch_state
{
  int stage, brackt;
  double ginit, gtest, gx, gy, finit, fx, fy, stx, sty,
      stmin, stmax, width, width1;
};



static void ts_mainlb(int n, int m, double *x,
                      ...
                      char *csave, int *isave, struct ts_mainlb_state *mainlb_state, struct ts_dcsrch_state *dcsrch_state);
...
static void dcsrch(double *, double *, double *,
                   ...
                   double, double, char *, struct ts_dcsrch_state *dcsrch_state);
...
static void lnsrlb(int, double *, double *,
                   ...
                   char *, struct ts_dcsrch_state *dcsrch_state);
...
static
void ts_setulb(int n, int m, double *x, double *l, double *u, int *nbd,
               double *f, double *g, double factr, double *pgtol,
               double *wa, int * iwa, char *task, int iprint, int *isave, struct ts_setulb_state *setulb_state, struct ts_mainlb_state *mainlb_state, struct ts_dcsrch_state *dcsrch_state)
{
/*     ************
...
*/
    ...
    if (strncmp(task, "START", 5) == 0) {
        setulb_state->lws  = 1;
        setulb_state->lwy  = setulb_state->lws + m * n;
        setulb_state->lsy  = setulb_state->lwy + m * n;
        setulb_state->lss  = setulb_state->lsy + m * m;
        setulb_state->lwt  = setulb_state->lss + m * m;
        setulb_state->lwn  = setulb_state->lwt + m * m;
        setulb_state->lsnd = setulb_state->lwn + (m * m << 2);
        setulb_state->lz   = setulb_state->lsnd + (m * m << 2);
        setulb_state->lr   = setulb_state->lz + n;
        setulb_state->ld   = setulb_state->lr + n;
        setulb_state->lt   = setulb_state->ld + n;
        setulb_state->lwa  = setulb_state->lt + n;
    }

    ts_mainlb(n, m, x, l, u, nbd, f, g, factr, pgtol,
              &wa[setulb_state->lws], &wa[setulb_state->lwy], &wa[setulb_state->lsy],&wa[setulb_state->lss], &wa[setulb_state->lwt],&wa[setulb_state->lwn],
              &wa[setulb_state->lsnd], &wa[setulb_state->lz], &wa[setulb_state->lr], &wa[setulb_state->ld], &wa[setulb_state->lt], &wa[setulb_state->lwa],
              iwa, &iwa[n], &iwa[n << 1], task, iprint,
              csave, isave, mainlb_state, dcsrch_state);
    return;
} /* setulb */
/* ======================= The end of setulb ============================= */
...
/*          Generate the current machine precision. */
        mainlb_state->epsmch = DBL_EPSILON;
        mainlb_state->fold = 0.;
        mainlb_state->dnorm = 0.;
        mainlb_state->gd = 0.;
        mainlb_state->sbgnrm = 0.;
        mainlb_state->stp = 0.;
        xstep = 0.;
        mainlb_state->stpmx = 0.;
        mainlb_state->gdold = 0.;
        mainlb_state->dtd = 0.;
/*          Initialize counters and scalars when task='START'. */
/*             for the limited memory BFGS matrices: */
        mainlb_state->col = 0;
        mainlb_state->head = 1;
        mainlb_state->theta = 1.;
        mainlb_state->iupdat = 0;
        mainlb_state->updatd = FALSE_;
        mainlb_state->iback = 0;
        mainlb_state->itail = 0;
        mainlb_state->ifun = 0;
        mainlb_state->iword = 0;
        mainlb_state->nact = 0;
        mainlb_state->ileave = 0;
        mainlb_state->nenter = 0;
/*             for operation counts: */
        mainlb_state->iter = 0;
        mainlb_state->nfgv = 0;
        mainlb_state->nint = 0;
        mainlb_state->nintol = 0;
        mainlb_state->nskip = 0;
        mainlb_state->nfree = n;
/*             for stopping tolerance: */
        mainlb_state->tol = factr * mainlb_state->epsmch;
/*             'word' records the status of subspace solutions. */
        strcpy(word, "---");
/*             'info' records the termination information. */
        mainlb_state->info = 0;
/*          Check the input arguments for errors. */
        errclb(n, m, factr, &l[1], &u[1], &nbd[1], task, &mainlb_state->info, &k);
        if (strncmp(task, "ERROR", 5) == 0) {
            prn3lb(n, x+1, f, task, iprint, mainlb_state->info, mainlb_state->iter, mainlb_state->nfgv, mainlb_state->nintol, mainlb_state->nskip,
                   mainlb_state->nact, mainlb_state->sbgnrm, mainlb_state->nint, word, mainlb_state->iback, mainlb_state->stp, xstep, k);
            return;
        }

        prn1lb(n, m, l+1, u+1, x+1, iprint, mainlb_state->epsmch);

/*          Initialize iwhere & project x onto the feasible set. */
        active(n, &l[1], &u[1], &nbd[1], &x[1], &iwhere[1], iprint, &mainlb_state->prjctd,
                &mainlb_state->cnstnd, &mainlb_state->boxed);
...
/*     Compute the infinity norm of the (-) projected gradient. */
    projgr(n, &l[1], &u[1], &nbd[1], &x[1], &g[1], &mainlb_state->sbgnrm);

    if (iprint >= 1)
        ts_printf("At iterate %5d  f= %12.5g  |proj g|= %12.5g\n",
                  mainlb_state->iter, *f, mainlb_state->sbgnrm);

    if (mainlb_state->sbgnrm <= *pgtol) {
...
L222:
    if (iprint >= 99) ts_printf("Iteration %5d\n", mainlb_state->iter);
    mainlb_state->iword = -1;

    if (! mainlb_state->cnstnd && mainlb_state->col > 0) {
/*                                              skip the search for GCP. */
        F77_CALL(dcopy)(&n, &x[1], &c__1, &z[1], &c__1);
        wrk = mainlb_state->updatd;
        mainlb_state->nint = 0;
        goto L333;
    }
...
/* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    cauchy(n, &x[1], &l[1], &u[1], &nbd[1], &g[1], &indx2[1], &iwhere[1], &t[
            ...
            sy_offset], &wt[wt_offset], &mainlb_state->theta, &mainlb_state->col, &mainlb_state->head, &wa[1], &wa[(m
            << 1) + 1], &wa[(m << 2) + 1], &wa[m * 6 + 1], &mainlb_state->nint, iprint, &
            mainlb_state->sbgnrm, &mainlb_state->info, &mainlb_state->epsmch);
    if (mainlb_state->info != 0) {
/*           singular triangular system detected; refresh the lbfgs memory. */
    ...
    mainlb_state->nintol += mainlb_state->nint;
/*     Count the entering and leaving variables for iter > 0; */
/*     find the index set of free and active variables at the GCP. */
    freev(n, &mainlb_state->nfree, &indx[1], &mainlb_state->nenter, &mainlb_state->ileave, &indx2[1], &iwhere[1], &
            wrk, &mainlb_state->updatd, &mainlb_state->cnstnd, iprint, &mainlb_state->iter);
    mainlb_state->nact = n - mainlb_state->nfree;
L333:
/*     If there are no free variables or B=theta*I, then */
/*                                          skip the subspace minimization. */
    if (mainlb_state->nfree == 0 || mainlb_state->col == 0) {
        goto L555;
    }
/* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
...
/*                       [ 0  I] */
    if (wrk)
        formk(n, &mainlb_state->nfree, &indx[1], &mainlb_state->nenter, &mainlb_state->ileave, &indx2[1], &mainlb_state->iupdat, &
                mainlb_state->updatd, &wn[wn_offset], &snd[snd_offset], m, &ws[ws_offset], &
                wy[wy_offset], &sy[sy_offset], &mainlb_state->theta, &mainlb_state->col, &mainlb_state->head, &mainlb_state->info);
    if (mainlb_state->info != 0) {
...
/*                                                     from 'cauchy'). */
    cmprlb(n, m, &x[1], &g[1], &ws[ws_offset], &wy[wy_offset],
           ...
           &mainlb_state->theta, &mainlb_state->col, &mainlb_state->head, &mainlb_state->nfree, &mainlb_state->cnstnd, &mainlb_state->info);
    if (mainlb_state->info != 0)
        goto L444;
/*         call the direct method. */
    subsm(n, m, &mainlb_state->nfree, &indx[1], &l[1], &u[1], &nbd[1], &z[1], &r[1], &
            ws[ws_offset], &wy[wy_offset], &mainlb_state->theta, &mainlb_state->col, &mainlb_state->head, &mainlb_state->iword, &wa[1]
            , &wn[wn_offset], iprint, &mainlb_state->info);
L444:
    if (mainlb_state->info != 0) {
/*            singular triangular system detected; */
/*            refresh the lbfgs memory and restart the iteration. */
...
/*     Generate the search direction d:=z-x. */
    for (int i = 1; i <= n; ++i)
        d[i] = z[i] - x[i];
L666:
    lnsrlb(n, &l[1], &u[1], &nbd[1], &x[1], f, &mainlb_state->fold, &mainlb_state->gd, &mainlb_state->gdold, &g[1], &
            d[1], &r[1], &t[1], &z[1], &mainlb_state->stp, &mainlb_state->dnorm, &mainlb_state->dtd, &xstep, &
            mainlb_state->stpmx, &mainlb_state->iter, &mainlb_state->ifun, &mainlb_state->iback, &mainlb_state->nfgv, &mainlb_state->info, task, &mainlb_state->boxed, &mainlb_state->cnstnd,
            csave, dcsrch_state);
    if (mainlb_state->info != 0 || mainlb_state->iback >= 20) {
/*            restore the previous iterate. */
        F77_CALL(dcopy)(&n, &t[1], &c__1, &x[1], &c__1);
        F77_CALL(dcopy)(&n, &r[1], &c__1, &g[1], &c__1);
        *f = mainlb_state->fold;
        if (mainlb_state->col == 0) {
...
/*          Print iteration information. */
        prn2lb(n, x+1, f, g+1, iprint, mainlb_state->iter, mainlb_state->nfgv, mainlb_state->nact,
               mainlb_state->sbgnrm, mainlb_state->nint, word, mainlb_state->iword, mainlb_state->iback, mainlb_state->stp, xstep);
        goto L1000;
    }
L777:
...
/* Computing MAX */
    d__1 = fabs(mainlb_state->fold), d__2 = fabs(*f), d__1 = max(d__1, d__2);
    ddum = max(d__1, 1.);
    if (mainlb_state->fold - *f <= mainlb_state->tol * ddum) {
/*                                          terminate the algorithm. */
        strcpy(task, "CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH");
        if (mainlb_state->iback >= 10) mainlb_state->info = -5;
...
/*     Compute d=newx-oldx, r=newg-oldg, rr=y'y and dr=y's. */
    ...
    if (mainlb_state->stp == 1.) {
        dr = mainlb_state->gd - mainlb_state->gdold;
        ddum = -mainlb_state->gdold;
    } else {
        dr = (mainlb_state->gd - mainlb_state->gdold) * mainlb_state->stp;
        F77_CALL(dscal)(&n, &mainlb_state->stp, &d[1], &c__1);
        ddum = -mainlb_state->gdold * mainlb_state->stp;
    }
    if (dr <= mainlb_state->epsmch * ddum) {
/*                              skip the L-BFGS update. */
        ++mainlb_state->nskip;
        mainlb_state->updatd = FALSE_;
        if (iprint >= 1)
            ts_printf("ys=%10.3e  -gs=%10.3e, BFGS update SKIPPED\n", dr, ddum);
        goto L888;
    }
/* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
...
/* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    mainlb_state->updatd = TRUE_;
    ++mainlb_state->iupdat;
/*     Update matrices WS and WY and form the middle matrix in B. */
    matupd(n, m, &ws[ws_offset], &wy[wy_offset], &sy[sy_offset], &ss[
            ss_offset], &d[1], &r[1], &mainlb_state->itail, &mainlb_state->iupdat, &mainlb_state->col, &mainlb_state->head, &
            mainlb_state->theta, &rr, &dr, &mainlb_state->stp, &mainlb_state->dtd);
/*     Form the upper half of the pds T = theta*SS + L*D^(-1)*L'; */
/*          Store T in the upper triangular of the array wt; */
/*          Cholesky factorize T to J*J' with */
/*             J' stored in the upper triangular of wt. */
    formt(m, &wt[wt_offset], &sy[sy_offset], &ss[ss_offset], &mainlb_state->col, &mainlb_state->theta, &
            mainlb_state->info);
    if (mainlb_state->info != 0) {
/*            nonpositive definiteness in Cholesky factorization; */
/*            refresh the lbfgs memory and restart the iteration. */
...
//    isave[1-1] = mainlb_state->nintol;
//    isave[3-1] = mainlb_state->itfile;
//    isave[4-1] = mainlb_state->iback;
//    isave[5-1] = mainlb_state->nskip;
//    isave[6-1] = mainlb_state->head;
//    isave[7-1] = mainlb_state->col;
//    isave[8-1] = mainlb_state->itail;
//    isave[9-1] = mainlb_state->iter;
//    isave[10-1] = mainlb_state->iupdat;
//    isave[12-1] = mainlb_state->nint;
    isave[13-1] = mainlb_state->nfgv;
//    isave[14-1] = mainlb_state->info;
//    isave[15-1] = mainlb_state->ifun;
//    isave[16-1] = mainlb_state->iword;
//    isave[17-1] = mainlb_state->nfree;
//    isave[18-1] = mainlb_state->nact;
//    isave[19-1] = mainlb_state->ileave;
//    isave[20-1] = mainlb_state->nenter;

    prn3lb(n, x+1, f, task, iprint, mainlb_state->info, mainlb_state->iter, mainlb_state->nfgv, mainlb_state->nintol, mainlb_state->nskip, mainlb_state->nact,
           mainlb_state->sbgnrm, mainlb_state->nint, word, mainlb_state->iback, mainlb_state->stp, xstep, k);
    return;
} /* mainlb */
/* ======================= The end of mainlb ============================= */
...
/*                     this variable is always fixed */
    ...
    if (iprint >= 0) {
        if (*prjctd)
            ts_printf("The initial X is infeasible.  Restart with its projection.\n");
        if (!*cnstnd) ts_printf("This problem is unconstrained.\n");
    }
    if (iprint > 0)
        ts_printf("At X0, %d variables are exactly at the bounds\n", nbdd);
 ...
 */
    ...
    if (*sbgnrm <= 0.) {
        if (iprint >= 0) ts_printf("Subgnorm = 0.  GCP = X.\n");
...
/*                    is a zero vector, return with the initial xcp as GCP. */
        if (iprint > 100) {
            ts_printf("Cauchy X =  ");
            for (int i = 1; i <= n; i++) ts_printf("%g ", xcp[i]);
            ts_printf("\n");
        }
        return;
    }
...
/*     Initialize derivative f2. */
    ...
    tsum = 0.;
    *nint = 1;
    if (iprint >= 99) ts_printf("There are %d  breakpoints\n", nbreak);
...
/*           cost of heapsort is avoided. */
    ...
    if (dt != 0 && iprint >=  100) {
        ts_printf("\nPiece    %3i f1, f2 at start point %11.4e %11.4e\n",
                  *nint, f1, f2);
        ts_printf("Distance to the next break point =  %11.4e\n", dt);
        ts_printf("Distance to the stationary point =  %11.4e\n", dtm);
    }
...
/*         reset the corresponding component of d to zero. */
    ...
    if (iprint >= 100) ts_printf("Variable  %d  is fixed.\n", ibp);
    if (nleft == 0 && nbreak == n) {
...
L888:
    if (iprint >= 99) {
        ...
        ts_printf("Piece    %3i f1, f2 at start point %11.4e %11.4e\n",
                  *nint,f1,f2);
        ts_printf("Distance to the stationary point =  %11.4e\n", dtm);
    }
    ...
    if (*iter > 0 && *cnstnd) {/* count the entering and leaving variables. */
        for (int i = 1; i <= *nfree; ++i) {
            ...
            if (iwhere[k] > 0) {
                ...
                indx2[*ileave] = k;
                if (iprint >= 100)
                    ts_printf("Variable %d leaves the set of free variables\n",
                              k);
            }
        }
        for (int i = *nfree + 1; i <= n; ++i) {
            ...
            if (iwhere[k] <= 0) {
                ...
                indx2[*nenter] = k;
                if (iprint >= 100)
                    ts_printf("Variable %d enters the set of free variables\n",
                              k);
            }
         if (iprint >= 100)
             ts_printf("%d variables leave; %d variables enter\n",
                       n + 1 - *ileave, *nenter);
        }
    }
...
/*     Find the index set of free and active variables at the GCP. */
    *nfree = 0;
    ...
    if (iprint >= 99)
        ts_printf("%d  variables are free at GCP on iteration %d\n",
                  *nfree, *iter + 1);
    return;
} /* freev */
/* ======================= The end of freev ============================== */
...
static void lnsrlb(int n, double *l, double *u,
                   ...
                   char *csave, struct ts_dcsrch_state *dcsrch_state)
{
/*     **********
...
/*                                 Line search is impossible. */
    ...
    dcsrch(f, gd, stp, ftol, gtol, xtol, stpmin, *stpmx, csave, dcsrch_state);
    *xstep = *stp * *dnorm;
...
static void dcsrch(double *f, double *g, double *stp,
                   ...
                   double stpmin, double stpmax, char *task, struct ts_dcsrch_state *dcsrch_state)
{
/*        **********
...
/*          Initialize local variables. */
        dcsrch_state->brackt = FALSE_;
        dcsrch_state->stage = 1;
        dcsrch_state->finit = *f;
        dcsrch_state->ginit = *g;
        dcsrch_state->gtest = ftol * dcsrch_state->ginit;
        dcsrch_state->width = stpmax - stpmin;
        dcsrch_state->width1 = dcsrch_state->width / .5;
...
/*          function, and derivative at stp. */
        dcsrch_state->stx = 0.;        dcsrch_state->fx = dcsrch_state->finit;        dcsrch_state->gx = dcsrch_state->ginit;
        dcsrch_state->sty = 0.;        dcsrch_state->fy = dcsrch_state->finit;        dcsrch_state->gy = dcsrch_state->ginit;
        dcsrch_state->stmin = 0.;
        dcsrch_state->stmax = *stp + *stp * 4.;
        strcpy(task, "FG");
        return;
    }
/*      If psi(stp) <= 0 and f'(stp) >= 0 for some step, then the */
/*      algorithm enters the second stage. */
    ftest = dcsrch_state->finit + *stp * dcsrch_state->gtest;
    if (dcsrch_state->stage == 1 && *f <= ftest && *g >= 0.)
        dcsrch_state->stage = 2;
/*        Test for warnings. */
    if (dcsrch_state->brackt && (*stp <= dcsrch_state->stmin || *stp >= dcsrch_state->stmax))
        strcpy(task, "WARNING: ROUNDING ERRORS PREVENT PROGRESS");
    if (dcsrch_state->brackt && dcsrch_state->stmax - dcsrch_state->stmin <= xtol * dcsrch_state->stmax)
        strcpy(task, "WARNING: XTOL TEST SATISFIED");
    if (*stp == stpmax && *f <= ftest && *g <= dcsrch_state->gtest)
        strcpy(task, "WARNING: STP = STPMAX");
    if (*stp == stpmin && (*f > ftest || *g >= dcsrch_state->gtest))
        strcpy(task, "WARNING: STP = STPMIN");
/*        Test for convergence. */
    if (*f <= ftest && fabs(*g) <= gtol * (-dcsrch_state->ginit))
...
/*     the decrease is not sufficient. */
    if (dcsrch_state->stage == 1 && *f <= dcsrch_state->fx && *f > ftest) {
/*          Define the modified function and derivative values. */
        fm = *f - *stp * dcsrch_state->gtest;
        fxm = dcsrch_state->fx - dcsrch_state->stx * dcsrch_state->gtest;
        fym = dcsrch_state->fy - dcsrch_state->sty * dcsrch_state->gtest;
        gm = *g - dcsrch_state->gtest;
        gxm = dcsrch_state->gx - dcsrch_state->gtest;
        gym = dcsrch_state->gy - dcsrch_state->gtest;
/*          Call dcstep to update stx, sty, and to compute the new step. */
        dcstep(&dcsrch_state->stx, &fxm, &gxm, &dcsrch_state->sty, &fym, &gym, stp, &fm, &gm, &dcsrch_state->brackt, &
                dcsrch_state->stmin, &dcsrch_state->stmax);
/*          Reset the function and derivative values for f. */
        dcsrch_state->fx = fxm + dcsrch_state->stx * dcsrch_state->gtest;
        dcsrch_state->fy = fym + dcsrch_state->sty * dcsrch_state->gtest;
        dcsrch_state->gx = gxm + dcsrch_state->gtest;
        dcsrch_state->gy = gym + dcsrch_state->gtest;
    } else {
/*         Call dcstep to update stx, sty, and to compute the new step. */
        dcstep(&dcsrch_state->stx, &dcsrch_state->fx, &dcsrch_state->gx, &dcsrch_state->sty, &dcsrch_state->fy, &dcsrch_state->gy, stp, f, g, &dcsrch_state->brackt, &dcsrch_state->stmin, &
                dcsrch_state->stmax);
    }
/*     Decide if a bisection step is needed. */
    if (dcsrch_state->brackt) {
        if (fabs(dcsrch_state->sty - dcsrch_state->stx) >= dcsrch_state->width1 * .66) {
            *stp = dcsrch_state->stx + (dcsrch_state->sty - dcsrch_state->stx) * .5;
        }
        dcsrch_state->width1 = dcsrch_state->width;
        dcsrch_state->width = fabs(dcsrch_state->sty - dcsrch_state->stx);
    }
/*     Set the minimum and maximum steps allowed for stp. */
    if (dcsrch_state->brackt) {
        dcsrch_state->stmin = min(dcsrch_state->stx,dcsrch_state->sty);
        dcsrch_state->stmax = max(dcsrch_state->stx,dcsrch_state->sty);
    } else {
        dcsrch_state->stmin = *stp + (*stp - dcsrch_state->stx) * 1.1;
        dcsrch_state->stmax = *stp + (*stp - dcsrch_state->stx) * 4.;
    }
...
/*     point obtained during the search. */
    if ((dcsrch_state->brackt && (*stp <= dcsrch_state->stmin || *stp >= dcsrch_state->stmax)) ||
        (dcsrch_state->brackt && (dcsrch_state->stmax - dcsrch_state->stmin <= xtol * dcsrch_state->stmax))) {
        *stp = dcsrch_state->stx;
    }
...
{
    ts_printf("%s ", title);
    for (int i = 0; i < n; i++) ts_printf("%g ", x[i]);
    ts_printf("\n");
}
...
{
    if (iprint >=  0) {
        ts_printf("N = %d, M = %d machine precision = %g\n", n, m, epsmch);
        if (iprint >= 100){
            pvector("L =", l, n);
            pvector("X0 =",x, n);
            pvector("U =", u, n);
...
{
    if (iprint >=  99) {
        ts_printf("LINE SEARCH %d times; norm of step = %g\n", iback, xstep);
        if (iprint > 100) {
            pvector("X =", x, n);
            pvector("G =", g, n);
        }
                   ...
                   int k)
{
    if(strncmp(task, "CONV", 4) == 0) {
        ...
        if (iprint >= 1) ts_printf("F = %g\n", *f);
    }
    if (iprint >= 0) {
        switch(info) {
        case -1: ts_printf("Matrix in 1st Cholesky factorization in formk is not Pos. Def."); break;
        case -2: ts_printf("Matrix in 2st Cholesky factorization in formk is not Pos. Def."); break;
        case -3: ts_printf("Matrix in the Cholesky factorization in formt is not Pos. Def."); break;
        case -4: ts_printf("Derivative >= 0, backtracking line search impossible."); break;
        case -5: ts_printf("l(%d) > u(%d).  No feasible solution", k, k); break;
        case -6: ts_printf("Input nbd(%d) is invalid", k); break;
        case -7: ts_printf("Warning:  more than 10 function and gradient evaluations\n   in the last line search\n"); break;
        case -8: ts_printf("The triangular system is singular."); break;
